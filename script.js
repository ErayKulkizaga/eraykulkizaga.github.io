document.addEventListener('DOMContentLoaded', () => {
  ...
  const scrollFilm = scrollStory?.querySelector('[data-scroll-film]');
  const storyProgress = scrollStory?.querySelector('[data-story-progress]');
  if (scrollStory && scrollFilm instanceof HTMLVideoElement) {
    const reducedMotion = window.matchMedia('(prefers-reduced-motion: reduce)').matches;
    let targetProgress = 0;
    let renderedProgress = 0;
    let duration = Math.max(0, Number(scrollFilm.dataset.duration || 0) - 0.04);
    let storyFrame = 0;

    const clampStoryProgress = (value) => Math.min(1, Math.max(0, value));
    const measureStory = () => {
      const bounds = scrollStory.getBoundingClientRect();
      const scrollable = Math.max(1, scrollStory.offsetHeight - window.innerHeight);
      targetProgress = clampStoryProgress(-bounds.top / scrollable);
      scrollStory.style.setProperty('--story-progress', targetProgress.toFixed(4));
      if (storyProgress instanceof HTMLElement) {
        storyProgress.style.width = `${(targetProgress * 100).toFixed(2)}%`;
      }
    };

    const renderStory = () => {
      renderedProgress += (targetProgress - renderedProgress) * 0.14;
      const requestedTime = renderedProgress * duration;
      if (duration && Math.abs(scrollFilm.currentTime - requestedTime) > 0.012) {
        scrollFilm.currentTime = requestedTime;
      }
    ...
    const updateStory = () => {
      measureStory();
      if (!reducedMotion && !storyFrame) storyFrame = window.requestAnimationFrame(renderStory);
    };

    let filmInitialized = false;
    let filmObjectUrl = '';
    const initializeStoryFilm = async () => {
      if (filmInitialized) return;
      filmInitialized = true;
      const authoredDuration = Number(scrollFilm.dataset.duration);
      duration = Number.isFinite(scrollFilm.duration)
        ? Math.max(0, scrollFilm.duration - 0.04)
        : Math.max(0, authoredDuration - 0.04);
      try {
        await scrollFilm.play();
      } catch {
        // The poster remains visible when a browser blocks autoplay.
      }
      scrollFilm.pause();
      scrollFilm.currentTime = 0.001;
      updateStory();
    };

    const prepareSeekableFilm = async () => {
      const source = scrollFilm.querySelector('source')?.src || scrollFilm.currentSrc;
      try {
        const response = await fetch(source, { cache: 'force-cache' });
        if (!response.ok) throw new Error(`Video request failed: ${response.status}`);
        filmObjectUrl = URL.createObjectURL(await response.blob());
        scrollFilm.src = filmObjectUrl;
        scrollFilm.load();
      } catch {
        // Keep the original URL and poster as the network fallback.
      }

      if (scrollFilm.readyState >= 1) initializeStoryFilm();
      else scrollFilm.addEventListener('loadedmetadata', initializeStoryFilm, { once: true });
    };
    ...
    window.addEventListener('pagehide', () => {
      if (filmObjectUrl) URL.revokeObjectURL(filmObjectUrl);
    }, { once: true });
    updateStory();
  }
